...
object UserOperations {
  ...
  def writeAvroGenotypeFile(file: File, genotypes: Array[Genotype]): Unit =
  {
    val prototype = genotypes(0)
    val datumWriter = new SpecificDatumWriter[Genotype](
      prototype.getClass.asInstanceOf[java.lang.Class[Genotype]])
    val dataFileWriter = new DataFileWriter[Genotype](datumWriter)

    dataFileWriter.create(prototype.getSchema, file)

    for (i <- 0 until genotypes.length by 1)
    {
      dataFileWriter.append(genotypes(i))
    }
    dataFileWriter.close()
  }
